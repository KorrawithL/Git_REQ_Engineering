...
# ==========================================================
# 📊 3. ฟังก์ชัน Popup ดูสรุปรายงาน (View Summary Dialog)
# ==========================================================
                ...
                try:
                    ...
                    with conn.cursor() as cur:
                        if b_id_t1 == "ทั้งหมด":
                            ...
                            cur.execute(sql, tuple(user_allowed_branches) + (start_date_t1, end_date_t1))
                        else:
                    ...
                    if raw_data_t1:
                        ...
                        with st.container(height=380, border=False):
                            for r in raw_data_t1:
                                ...
                                if current_role == 'admin':
                                    ...
                                    if str(r.get('branch_id')) in user_allowed_branches:
                                        can_crud = True
                        ...
                        # 🎯 กรองเอาเฉพาะเครื่องจักรที่มีการบันทึกข้อมูลในช่วงเวลานี้มาแสดงเท่านั้น
                        active_machines_config = [m for m in machines_config if any([matrix_data[d][m['name']]['has_data'] for d in range(1, 32)])]
                        if not active_machines_config: # กรณีไม่มีเครื่องไหนตรงเลย ให้ fallback ใช้ทั้งหมด (กัน error)
                            active_machines_config = machines_config
...
<html><head><meta charset="utf-8"><title>Print Summary Machine Report</title><style>@page {{ size: A4 landscape; margin: 4mm; }} body {{ font-family: 'Sarabun', Tahoma, sans-serif; margin: 0; padding: 5px; background-color: #FFFFFF; color: #000000; }} .header-title {{ text-align: center; font-size: 16px; font-weight: bold; margin-bottom: 4px; }} .banner {{ background-color: #F8CBAD; text-align: center; font-size: 14px; font-weight: bold; padding: 5px; border: 1px solid #000; margin-bottom: 4px; }} table {{ width: 100%; border-collapse: collapse; }} th, td {{ font-family: 'Sarabun', Tahoma, sans-serif; color: #000000 !important; }}</style></head><body><div class="header-title">บริษัท วู้ดเวิร์ค จำกัด สาขา {selected_branch_display}</div><div class="banner">สรุปชั่วโมงการทำงานของเครื่องจักร/เบรกดาวน์ ประจำเดือน....{month_str}................... {year_buddhist}</div><table><thead><tr><th rowspan="2" style="background-color:#E2EFDA;border:1px solid #000;padding:3px;font-size:10px;width:35px;color:#000000 !important;">วันที่</th>{header_row1}</tr><tr>{header_row2}</tr></thead><tbody>{body_rows}{total_row_html}{pct_row_html}</tbody></table></body></html>"""
                        ...
                        with col_btn2:
                            if st.button("📊 สรุปรายงาน", use_container_width=True, key="view_t1"):
                                show_summary_report_dialog(table_full_html)
                        with col_btn3:
                ...
                try:
                    ...
                    with conn.cursor() as cur:
                        if b_id_t2 == "ทั้งหมด":
                            ...
                            sql_trans = f"""SELECT r.*, b.branch_name 
                                           FROM fuel_records r LEFT JOIN branches b ON r.branch_id = b.id
                                           WHERE r.branch_id IN ({placeholders}) AND r.record_date BETWEEN %s AND %s ORDER BY r.record_date ASC"""
                            cur.execute(sql_trans, tuple(user_allowed_branches) + (start_date_t2, end_date_t2))
                            raw_data_t2 = cur.fetchall()
                        else:
                            sql_eng = """SELECT e.id AS engine_pk_id, CONVERT(e.engine_code USING utf8mb4) AS engine_code, CONVERT(et.type_name USING utf8mb4) AS type_name, CONVERT(b.branch_name USING utf8mb4) AS branch_name, e.branch_id
...
<html><head><meta charset="utf-8"><title>Print Fuel Report Matrix</title><style>@page {{ size: A4 landscape; margin: 4mm; }} body {{ font-family: 'Sarabun', Tahoma, sans-serif; margin: 0; padding: 5px; background-color: #FFFFFF; color: #000000; }} .header-title {{ text-align: center; font-size: 16px; font-weight: bold; margin-bottom: 4px; }} .banner {{ text-align: center; font-size: 14px; font-weight: bold; padding: 5px; margin-bottom: 4px; }} table {{ width: 100%; border-collapse: collapse; }} th, td {{ font-family: 'Sarabun', Tahoma, sans-serif; color: #000000 !important; }}</style></head><body><div class="header-title">บริษัท วู้ดเวิร์ค จำกัด สาขา {selected_branch_display}</div><div class="banner">รายงานการใช้เชื้อเพลิงของรถ ประจำเดือน {month_str} {year_buddhist}</div><table><thead><tr><th rowspan="3" style="border:1px solid #000;padding:3px;font-size:10px;width:35px;">วันที่</th>{header_row1_t2}</tr><tr>{header_row2_t2}</tr><tr>{header_row3_t2}</tr></thead><tbody>{body_rows_t2}</tbody></table></body></html>"""
                        ...
                        with col_btn2:
                            if st.button("📊 สรุปรายงาน", use_container_width=True, key="view_t2"):
                                show_summary_report_dialog(table_full_html_t2)
                        with col_btn3:
                ...
                try:
                    ...
                    with conn.cursor() as cur:
                        if b_id_t3 == "ทั้งหมด":
                            ...
                            cur.execute(sql, tuple(user_allowed_branches) + (start_date_t3, end_date_t3))
                        else:
                    ...
                    if raw_data_t3:
                        ...
                        for r in raw_data_t3:
                            ...
                            for b in branches_config:
                                if b['code'].lower() == b_code.lower():
                                    matrix_data_t3[d_num][b['code']] = {'total': float(r.get('total_count') or 0.0), 'drop': float(r.get('total_drop') or 0.0), 'pm': float(r.get('pm_drop') or 0.0), 'non_pm': float(r.get('non_pm_drop') or 0.0), 'remark': str(r.get('remark') or '').strip(), 'has_data': True}
                                    break
                        ...
                        for b in branches_config:
                            ...
                            branch_totals_t3[b_code] = {'total': tot_c, 'drop': tot_d, 'pm': pm_d, 'non_pm': npm_d, 'p_tot': f"{(tot_d/tot_c*100):.2f}%" if tot_c>0 else "0.00%", 'p_pm': f"{(pm_d/tot_c*100):.2f}%" if tot_c>0 else "0.00%", 'p_npm': f"{(npm_d/tot_c*100):.2f}%" if tot_c>0 else "0.00%"}
...
<html><head><meta charset="utf-8"><title>Print Pressure Report Matrix</title><style>@page {{ size: A4 landscape; margin: 4mm; }} body {{ font-family: 'Sarabun', Tahoma, sans-serif; margin: 0; padding: 5px; background-color: #FFFFFF; color: #000000; }} .header-banner1 {{ background-color: #00FF00; color: #006100; text-align: center; font-size: 16px; font-weight: bold; padding: 6px; border: 1px solid #000; }} .header-banner2 {{ background-color: #FFFF00; color: #000; text-align: center; font-size: 14px; font-weight: bold; padding: 5px; border: 1px solid #000; margin-bottom: 4px; }} table {{ width: 100%; border-collapse: collapse; }} th, td {{ font-family: 'Sarabun', Tahoma, sans-serif; color: #000000 !important; }}</style></head><body><div class="header-banner1">รายงานการตกของ แรงดันไอน้ำปลายทาง ของบอยเลอร์ แบบเปรียบเทียบ</div><div class="header-banner2">ประจำเดือน {month_str} {year_buddhist}</div><table><thead><tr><th rowspan="3" style="background-color:#E000E0;color:#FFF;border:1px solid #000;padding:3px;font-size:10px;width:35px;color:#000000 !important;">วันที่</th>{header_row1_t3}</tr><tr>{header_row2_t3}</tr><tr>{header_row3_t3}</tr></thead><tbody>{body_rows_t3}</tbody></table></body></html>"""
                        ...
                        with col_btn2:
                            if st.button("📊 สรุปรายงาน", use_container_width=True, key="view_t3"):
                                show_summary_report_dialog(table_full_html_t3)
                        with col_btn3:
                ...
                try:
                    ...
                    with conn.cursor() as cur:
                        if b_id_t4 == "ทั้งหมด":
                            ...
                            cur.execute(sql, tuple(user_allowed_branches) + (start_date_t4, end_date_t4))
                        else:
                    ...
                    if raw_data_t4:
                        ...
                        for col, header in zip(header_cols, headers):
                            col.markdown(f"<span style='color:#64748B; font-weight:bold; font-size:13px;'>{header}</span>", unsafe_allow_html=True)
                        st.markdown("<hr style='margin: 0.2rem 0; border-color: #E2E8F0;'>", unsafe_allow_html=True)
