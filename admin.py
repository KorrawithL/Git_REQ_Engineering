...
def render_admin_user_management():
    st.header("👥 ระบบบริหารสิทธิ์และจัดการผู้ใช้งานระบบ (สิทธิ์ 1 แท็บต่อ 1 User)")
    ...
    with st.expander("➕ เพิ่มบัญชีผู้ใช้งานใหม่เข้าระบบโดยแอดมิน"):
        with st.form("admin_add_user_form", clear_on_submit=True):
            ...
            new_role = st.selectbox("กำหนดระดับสิทธิ์ (Role_tab)", options=["user", "manager", "admin"])
            
            btn_admin_create = st.form_submit_button("➕ เพิ่มข้อมูลผู้ใช้ระบบ")
            if btn_admin_create:
                if new_user == "" or new_pass == "":
                    ...
                    try:
                        ...
                        with connection.cursor() as cursor:
                            ...
                            if cursor.fetchone():
                                st.error("ชื่อ Username นี้มีอยู่ในระบบแล้ว")
                            else:
    ...
    if all_users:
        ...
        # 🎯 ดึงข้อมูลของผู้ใช้ที่เลือกออกมาใช้งานตรงๆ (ปลอดภัย 100%)
        current_user_row = [r for r in all_users if r['user_id'] == target_id][0]
        
        # 1. กำหนดตัวเลือกแท็บงาน (รวมตัวเลือก "เข้าถึงได้ทุกแท็บงาน")
        tab_assign_options = {
        ...
        # แสดงกล่องเลือกสิทธิ์ 3 คอลัมน์
        c_edit1, c_edit2, c_edit3 = st.columns(3)
        with c_edit1:
            edit_role = st.selectbox("ระดับสิทธิ์ (Role_tab):", options=role_list, index=curr_role_idx)
        with c_edit2:
            selected_tab_label = st.selectbox(
                "เลือกจัดสรรแท็บงานประจำบัญชี:",
                options=list(tab_assign_options.keys()),
                index=default_idx,
                key="admin_tab_assign_select"
            )
        with c_edit3:
            edit_status = st.selectbox("สถานะบัญชี:", options=status_list, index=curr_status_idx)

        # ดึงแท็บที่จะนำไปใช้
        selected_allowed_tabs = tab_assign_options[selected_tab_label]

        # ปุ่มบันทึกข้อมูลสิทธิ์
        if st.button("💾 บันทึกและอนุมัติสิทธิ์การเข้าถึงแท็บ", use_container_width=True):
            try:
                ...
                with conn.cursor() as cur:
                    # 🎯 แก้ไขชื่อตารางเป็น system_users และใช้ user_id ตาม DB ของคุณ
        ...
        with st.expander("🚨 โซนอันตราย: ลบบัญชีผู้ใช้นี้ออกจากระบบ"):
            ...
            btn_delete_user = st.button(f"🗑️ ยืนยันการลบตัวตน {selected_target_user}", key="btn_del_usr", type="primary", use_container_width=True)
            if btn_delete_user:
                try:
                    connection = get_db_connection()
                    with connection.cursor() as cursor:
                        ...
                        connection.commit()
                    connection.close()
                    st.error(f"ทำการลบบัญชี {selected_target_user} ออกจากฐานข้อมูลเสร็จสิ้น")
                    st.rerun()
